...
const ChatRoom = ({nameUser, nameRoom, connected, socket, initialMessages}) => {
  const [errorUsuarioExpulsado, setErrorUsuarioExpulsado] = useState("");
    const { message, setMessage, showMessage, sendMessage,setShowMessage} = MessageSocketIoRooms({ socket }, nameRoom, nameUser, setErrorUsuarioExpulsado);
  ...
  return (
    <>
      {/* Main Chat Section */}
      <div className="flex flex-col flex-auto h-full p-6 relative"> {/* Añadimos `relative` aquí */}
                  ...
                  {allMessages.map((msg, index) => (
                    ...
                    >
                      <div className={`flex items-center ${msg.sender.name === nameUser ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div
                          className={`relative ${msg.sender.name === nameUser ? 'mr-3' : 'ml-3'} text-sm bg-${
                            msg.sender === nameUser ? 'indigo' : 'white'
